...
//        new Object[] {"orc", 2, false},
  ...
  public static final TableSchema ID_TIME_FLINK_SCHEMA = TableSchema.builder()
          .field("id", DataTypes.BIGINT())
          .field("data", DataTypes.BIGINT())
          .field("update_time", DataTypes.STRING())
          .build();

  public static final Schema ID_TIME_SCHEMA = new Schema(
...
//  }
  ...
  public void setupTable() throws IOException {
    ...
    if (!partitioned) {
      ...
      table = create(ID_TIME_SCHEMA, PartitionSpec.builderFor(ID_TIME_SCHEMA).identity("id").build());
    }
...
////    properties.put("write.metadata.previous-versions-max", "5");
  ...
  public void testUpsertOnIdKeyAndQueryHistory() throws Exception {
    List<List<Row>> elementsPerCheckpoint = ImmutableList.of(
            ImmutableList.of(
                    row2("+I", 1, 101,"2022-08-25 15:44:52")
            ),
            ImmutableList.of(
    ...
    List<Object> expectedRecords = ImmutableList.of(
           1,
           0
    );
    ...
    testChangeLogsWithQueryHistory(ImmutableList.of("id"), row -> row.getField(0), true,
            elementsPerCheckpoint, expectedRecords,expressionList);

  }



  private void testChangeLogsWithQueryHistory(List<String> equalityFieldColumns,
                                              KeySelector<Row, Object> keySelector,
                                              boolean insertAsUpsert,
                                              List<List<Row>> elementsPerCheckpoint,
                                              List<Object> expectedRecordsPerCheckpoint,
                                              List<Expression> expressionList) throws Exception {
    DataStream<Row> dataStream = env.addSource(new BoundedTestSource<>(elementsPerCheckpoint), ID_TIME_TYPE_INFO);
